...
layout (location = 6) uniform mat4 gWVP;
layout (location = 7) uniform mat4 gWorld;
layout (location = 8) uniform mat4 gObject;
layout (location = 9) uniform float waveTime;
layout (location = 10) uniform bool drawGrid;
...
//  grid stuff
	gridFactor = 1.f * float( drawGrid==false ) + hexagonEquation( Position.x, Position.z ) * float( (positionOutput.y==0) ) * float(drawGrid==true);
//-----------

    positionOutput = (gObject * vec4(positionOutput + 0.02f * int(0) * normalOutput, 1.0)).xyz;

    float waveWidth = 2.f;
    float waveHeight = .15f;
    if ( positionOutput.y >= 0.f)
    {
	isSurface0 = 1.0f;
	positionOutput.y = -waveHeight;
	for ( int i = 0; i < numWaves; i ++ )
	{
	TexCoord0 += waves[i].waveDirection * waves[i].waveHeight * sin( waves[i].waveSpeed * waveTime ) / 20.f;
	positionOutput += 1.0f/numWaves * getWaveContribution(positionOutput.x,positionOutput.z,waveTime, waves[i]);
	Normal0 += 4*waves[i].waveHeight*getNormalContribution(positionOutput.x,positionOutput.z,waveTime, waves[i]);
	}
    }
